...
remove them (comment out) for the actual tournament with student-generated 'bots

example3 is the only (minimally) smart bot that actually uses the history
...
# Student Teams
# To include a team in the tournament, name it "team_XXXX.py"
# eg:. team_estabrizzle.py, team_niceguyz.py, team_niceguyzV2.python
# These will automatically be imported by the main program
# To MANUALLY add a team (eg., myBot.py), you can do the following:
# import myBot
# modules.append(myBot)

for mod in modules:
    importlib.reload(mod)
    #print ('reloaded',mod)
    for required_variable in ['team_name', 'strategy_name', 'strategy_description']:
        if not hasattr(mod, required_variable):
            setattr(mod, required_variable, 'missing assignment')
...
def main_play(modules):
    ...
    post_to_file(section0+section1+section2+'-'*80+'\nCODE REMOVED\n'+''.join(section3), filename=outfile)
...
def play_round(player1, player2, score1, score2, moves1, moves2):
    ...
    TREAT = 95 # (T, "temptation" in literature) when you betray your partner
    ...
    # Beyond serious (eg., "death") penalty for robots that
    # do not include a fail-safe and return invalid data
    ERROR = -1000
...
def make_section2(modules, scores):
    ...
    section2 += 'Rank Team name      (P#):  Score         with strategy name\n'
    section2 += '---- -------------- -----  ------------ -----------------------------\n'
...
#    exit()
    ...
    # Rockettes (P1):  -500 points with Backstabber
    for i, team in enumerate(section2_list):
        team_name, Pn, n_points, strategy_name = team
        section2 += '{:2d}    {:<15}({}): {:>7} points with {:<40}\n'.format(i+1, team_name[:15], Pn, n_points, strategy_name[:40])                       
    return section2 
